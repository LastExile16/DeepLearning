...
def train_one_epoch(train_dataloader, model: nn.Module, optimizer: opt.Optimizer, loss):
    """
...
def valid_one_epoch(valid_dataloader, model: nn.Module, loss):
    """
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data, target) in tqdm(
            ...
            # correct += torch.sum(torch.squeeze(pred.eq(target.data.view_as(pred))))
            total += data.size(0)
    v_acc = correct / total
...
def optimize(data_loaders, model, optimizer, loss, n_epochs, save_path, interactive_tracking=False):
    ...
    # YOUR CODE HERE
    scheduler  = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, threshold=0.01, verbose=True)
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if valid_loss_min is None or (
            ...
            ## when I was playing with hyperparameters, 
            # I forgot what dropout rate I have used for a certain model.
            # Thus, I use the following to store the model dropout rate along with other parameters
            dropout_layer = None
            ...
            # Extract the dropout rate if the dropout layer is found
            if dropout_layer is not None:
                dropout_rate = dropout_layer.p
            else:
                dropout_rate = None
            torch.save({
...
def one_epoch_test(test_dataloader, model: nn.Module, loss):
