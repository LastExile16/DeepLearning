...
## this function is not used in this example of imprive_cnn_solution.ipynb
def get_train_val_data_loaders(batch_size, valid_size, transforms, num_workers):
    ...
    shuffled_indices = torch.randperm(n_tot) # generate random numbers from 0 to n_tot
    train_idx, valid_idx = shuffled_indices[split:], shuffled_indices[:split]

    # define samplers for obtaining training and validation batches
    """
    random_split returns two Datasets with non-overlapping indices, which were 
    drawn randomly based on the passed lengths, while SubsetRandomSampler accepts 
    the indices directly. Both can be used for different use cases.
    E.g. if you want to run a training epoch with certain samples only (e.g. from a 
    subset of classes or “hard to learn” samples), you could calculate these indices 
    somehow and use a SubsetRandomSampler for this training epoch.

    random_split on the other hand splits the dataset randomly, 
    so that you wont have that much control over it.
